...
void setup() {
  ...
  TV.clear_screen();
  intro();
  TV.println("Sistema de video\nTeste para CFTV\n\n\nFeito por:\nFelipe Tavares\n\nDez 2016");
  TV.delay(6000);
  TV.clear_screen();
  TV.println("\n\n\nGraficos de teste...");
  TV.delay(2500);
  TV.clear_screen();
  TV.print(5, 44, "Formas Basicas...");
  TV.delay(2500);
...
while (digitalRead(botao) && flag)
{
  cube();
  if (!digitalRead(botao)) flag = 0x00;
}
while (digitalRead(botao) && !flag) {
  TV.clear_screen();
  TV.print(16, 40, "Voltagem: 12,2v");
  delay(50);
  if (!digitalRead(botao)) flag = 0x01;
}
}
void cube()
{
